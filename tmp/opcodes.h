/*
 * Auto-generated file. Do not edit manually.
 *
 * Generated by generate_qe6502_opcodes.py.
 */

#ifndef QE6502_OPCODES_H
#define QE6502_OPCODES_H

#include <stdint.h>

typedef enum {
    QE6502_ADDR_MODE_ILLEGAL = 0,
    QE6502_ADDR_MODE_ABSOLUTE,
    QE6502_ADDR_MODE_ABSOLUTE_X,
    QE6502_ADDR_MODE_ABSOLUTE_Y,
    QE6502_ADDR_MODE_ACCUMULATOR,
    QE6502_ADDR_MODE_IDX_INDIRECT_X,
    QE6502_ADDR_MODE_IMMEDIATE,
    QE6502_ADDR_MODE_IMPLIED,
    QE6502_ADDR_MODE_INDIRECT,
    QE6502_ADDR_MODE_INDIRECT_IDX_Y,
    QE6502_ADDR_MODE_INDIRECT_ZP,
    QE6502_ADDR_MODE_RELATIVE,
    QE6502_ADDR_MODE_ZP,
    QE6502_ADDR_MODE_ZP_RELATIVE,
    QE6502_ADDR_MODE_ZP_X,
    QE6502_ADDR_MODE_ZP_Y,
} qe6502_addr_mode_t;

typedef struct {
    const char *name;
    const char *description;
    const char *addr_mode_str;
    const void* reserved_ptr;
    uint8_t opcode;
    uint8_t bytes;
    uint8_t addr_mode;
    uint8_t is_cmos_extension;
    uint8_t reserved_data[4];
} qe6502_opcode_info_t;

static const qe6502_opcode_info_t qe6502_opcode_table[256] = {
    [0x00] = {
        .name = "BRK",
        .description = "Force Interrupt",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x00,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x01] = {
        .name = "ORA",
        .description = "Logical Inclusive OR",
        .addr_mode_str = "Idx Indirect X",
        .reserved_ptr = 0,
        .opcode = 0x01,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IDX_INDIRECT_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x02] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x02,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x03] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x03,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x04] = {
        .name = "TSB",
        .description = "Test and Set Bits",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0x04,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x05] = {
        .name = "ORA",
        .description = "Logical Inclusive OR",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0x05,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x06] = {
        .name = "ASL",
        .description = "Arithmetic Shift Left",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0x06,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x07] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x07,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x08] = {
        .name = "PHP",
        .description = "Push Processor Status",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x08,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x09] = {
        .name = "ORA",
        .description = "Logical Inclusive OR",
        .addr_mode_str = "Immediate",
        .reserved_ptr = 0,
        .opcode = 0x09,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IMMEDIATE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x0A] = {
        .name = "ASL",
        .description = "Arithmetic Shift Left",
        .addr_mode_str = "Accumulator",
        .reserved_ptr = 0,
        .opcode = 0x0A,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ACCUMULATOR,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x0B] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x0B,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x0C] = {
        .name = "TSB",
        .description = "Test and Set Bits",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x0C,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x0D] = {
        .name = "ORA",
        .description = "Logical Inclusive OR",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x0D,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x0E] = {
        .name = "ASL",
        .description = "Arithmetic Shift Left",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x0E,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x0F] = {
        .name = "BBR0",
        .description = "Branch on Bit Reset",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0x0F,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x10] = {
        .name = "BPL",
        .description = "Branch if Positive",
        .addr_mode_str = "Relative",
        .reserved_ptr = 0,
        .opcode = 0x10,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_RELATIVE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x11] = {
        .name = "ORA",
        .description = "Logical Inclusive OR",
        .addr_mode_str = "Indirect Idx Y",
        .reserved_ptr = 0,
        .opcode = 0x11,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_IDX_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x12] = {
        .name = "ORA",
        .description = "Logical Inclusive OR",
        .addr_mode_str = "Indirect ZP",
        .reserved_ptr = 0,
        .opcode = 0x12,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_ZP,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x13] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x13,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x14] = {
        .name = "TRB",
        .description = "Test and Reset Bits",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0x14,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x15] = {
        .name = "ORA",
        .description = "Logical Inclusive OR",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0x15,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x16] = {
        .name = "ASL",
        .description = "Arithmetic Shift Left",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0x16,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x17] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x17,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x18] = {
        .name = "CLC",
        .description = "Clear Carry Flag",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x18,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x19] = {
        .name = "ORA",
        .description = "Logical Inclusive OR",
        .addr_mode_str = "Absolute Y",
        .reserved_ptr = 0,
        .opcode = 0x19,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x1A] = {
        .name = "INC",
        .description = "Increment Accumulator",
        .addr_mode_str = "Accumulator",
        .reserved_ptr = 0,
        .opcode = 0x1A,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ACCUMULATOR,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x1B] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x1B,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x1C] = {
        .name = "TRB",
        .description = "Test and Reset Bits",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x1C,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x1D] = {
        .name = "ORA",
        .description = "Logical Inclusive OR",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0x1D,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x1E] = {
        .name = "ASL",
        .description = "Arithmetic Shift Left",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0x1E,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x1F] = {
        .name = "BBR1",
        .description = "Branch on Bit Reset",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0x1F,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x20] = {
        .name = "JSR",
        .description = "Jump to Subroutine",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x20,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x21] = {
        .name = "AND",
        .description = "Logical AND",
        .addr_mode_str = "Idx Indirect X",
        .reserved_ptr = 0,
        .opcode = 0x21,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IDX_INDIRECT_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x22] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x22,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x23] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x23,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x24] = {
        .name = "BIT",
        .description = "Bit Test",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0x24,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x25] = {
        .name = "AND",
        .description = "Logical AND",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0x25,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x26] = {
        .name = "ROL",
        .description = "Rotate Left",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0x26,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x27] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x27,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x28] = {
        .name = "PLP",
        .description = "Pull Processor Status",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x28,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x29] = {
        .name = "AND",
        .description = "Logical AND",
        .addr_mode_str = "Immediate",
        .reserved_ptr = 0,
        .opcode = 0x29,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IMMEDIATE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x2A] = {
        .name = "ROL",
        .description = "Rotate Left",
        .addr_mode_str = "Accumulator",
        .reserved_ptr = 0,
        .opcode = 0x2A,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ACCUMULATOR,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x2B] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x2B,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x2C] = {
        .name = "BIT",
        .description = "Bit Test",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x2C,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x2D] = {
        .name = "AND",
        .description = "Logical AND",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x2D,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x2E] = {
        .name = "ROL",
        .description = "Rotate Left",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x2E,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x2F] = {
        .name = "BBR2",
        .description = "Branch on Bit Reset",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0x2F,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x30] = {
        .name = "BMI",
        .description = "Branch if Minus",
        .addr_mode_str = "Relative",
        .reserved_ptr = 0,
        .opcode = 0x30,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_RELATIVE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x31] = {
        .name = "AND",
        .description = "Logical AND",
        .addr_mode_str = "Indirect Idx Y",
        .reserved_ptr = 0,
        .opcode = 0x31,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_IDX_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x32] = {
        .name = "AND",
        .description = "Logical AND",
        .addr_mode_str = "Indirect ZP",
        .reserved_ptr = 0,
        .opcode = 0x32,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_ZP,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x33] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x33,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x34] = {
        .name = "BIT",
        .description = "Bit Test",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0x34,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x35] = {
        .name = "AND",
        .description = "Logical AND",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0x35,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x36] = {
        .name = "ROL",
        .description = "Rotate Left",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0x36,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x37] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x37,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x38] = {
        .name = "SEC",
        .description = "Set Carry Flag",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x38,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x39] = {
        .name = "AND",
        .description = "Logical AND",
        .addr_mode_str = "Absolute Y",
        .reserved_ptr = 0,
        .opcode = 0x39,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x3A] = {
        .name = "DEC",
        .description = "Decrement Accumulator",
        .addr_mode_str = "Accumulator",
        .reserved_ptr = 0,
        .opcode = 0x3A,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ACCUMULATOR,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x3B] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x3B,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x3C] = {
        .name = "BIT",
        .description = "Bit Test",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0x3C,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x3D] = {
        .name = "AND",
        .description = "Logical AND",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0x3D,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x3E] = {
        .name = "ROL",
        .description = "Rotate Left",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0x3E,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x3F] = {
        .name = "BBR3",
        .description = "Branch on Bit Reset",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0x3F,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x40] = {
        .name = "RTI",
        .description = "Return from Interrupt",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x40,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x41] = {
        .name = "EOR",
        .description = "Exclusive OR",
        .addr_mode_str = "Idx Indirect X",
        .reserved_ptr = 0,
        .opcode = 0x41,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IDX_INDIRECT_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x42] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x42,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x43] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x43,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x44] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x44,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x45] = {
        .name = "EOR",
        .description = "Exclusive OR",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0x45,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x46] = {
        .name = "LSR",
        .description = "Logical Shift Right",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0x46,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x47] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x47,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x48] = {
        .name = "PHA",
        .description = "Push Accumulator",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x48,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x49] = {
        .name = "EOR",
        .description = "Exclusive OR",
        .addr_mode_str = "Immediate",
        .reserved_ptr = 0,
        .opcode = 0x49,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IMMEDIATE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x4A] = {
        .name = "LSR",
        .description = "Logical Shift Right",
        .addr_mode_str = "Accumulator",
        .reserved_ptr = 0,
        .opcode = 0x4A,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ACCUMULATOR,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x4B] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x4B,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x4C] = {
        .name = "JMP",
        .description = "Jump",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x4C,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x4D] = {
        .name = "EOR",
        .description = "Exclusive OR",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x4D,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x4E] = {
        .name = "LSR",
        .description = "Logical Shift Right",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x4E,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x4F] = {
        .name = "BBR4",
        .description = "Branch on Bit Reset",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0x4F,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x50] = {
        .name = "BVC",
        .description = "Branch if Overflow Clear",
        .addr_mode_str = "Relative",
        .reserved_ptr = 0,
        .opcode = 0x50,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_RELATIVE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x51] = {
        .name = "EOR",
        .description = "Exclusive OR",
        .addr_mode_str = "Indirect Idx Y",
        .reserved_ptr = 0,
        .opcode = 0x51,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_IDX_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x52] = {
        .name = "EOR",
        .description = "Exclusive OR",
        .addr_mode_str = "Indirect ZP",
        .reserved_ptr = 0,
        .opcode = 0x52,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_ZP,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x53] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x53,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x54] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x54,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x55] = {
        .name = "EOR",
        .description = "Exclusive OR",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0x55,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x56] = {
        .name = "LSR",
        .description = "Logical Shift Right",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0x56,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x57] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x57,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x58] = {
        .name = "CLI",
        .description = "Clear Interrupt Disable",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x58,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x59] = {
        .name = "EOR",
        .description = "Exclusive OR",
        .addr_mode_str = "Absolute Y",
        .reserved_ptr = 0,
        .opcode = 0x59,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x5A] = {
        .name = "PHY",
        .description = "Push Register Y",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x5A,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x5B] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x5B,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x5C] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x5C,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x5D] = {
        .name = "EOR",
        .description = "Exclusive OR",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0x5D,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x5E] = {
        .name = "LSR",
        .description = "Logical Shift Right",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0x5E,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x5F] = {
        .name = "BBR5",
        .description = "Branch on Bit Reset",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0x5F,
        .bytes = 4,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x60] = {
        .name = "RTS",
        .description = "Return from Subroutine",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x60,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x61] = {
        .name = "ADC",
        .description = "Add with Carry",
        .addr_mode_str = "Idx Indirect X",
        .reserved_ptr = 0,
        .opcode = 0x61,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IDX_INDIRECT_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x62] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x62,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x63] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x63,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x64] = {
        .name = "STZ",
        .description = "Store Zero",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0x64,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x65] = {
        .name = "ADC",
        .description = "Add with Carry",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0x65,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x66] = {
        .name = "ROR",
        .description = "Rotate Right",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0x66,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x67] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x67,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x68] = {
        .name = "PLA",
        .description = "Pull Accumulator",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x68,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x69] = {
        .name = "ADC",
        .description = "Add with Carry",
        .addr_mode_str = "Immediate",
        .reserved_ptr = 0,
        .opcode = 0x69,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IMMEDIATE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x6A] = {
        .name = "ROR",
        .description = "Rotate Right",
        .addr_mode_str = "Accumulator",
        .reserved_ptr = 0,
        .opcode = 0x6A,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ACCUMULATOR,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x6B] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x6B,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x6C] = {
        .name = "JMP",
        .description = "Jump",
        .addr_mode_str = "Indirect",
        .reserved_ptr = 0,
        .opcode = 0x6C,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x6D] = {
        .name = "ADC",
        .description = "Add with Carry",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x6D,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x6E] = {
        .name = "ROR",
        .description = "Rotate Right",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x6E,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x6F] = {
        .name = "BBR6",
        .description = "Branch on Bit Reset",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0x6F,
        .bytes = 5,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x70] = {
        .name = "BVS",
        .description = "Branch if Overflow Set",
        .addr_mode_str = "Relative",
        .reserved_ptr = 0,
        .opcode = 0x70,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_RELATIVE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x71] = {
        .name = "ADC",
        .description = "Add with Carry",
        .addr_mode_str = "Indirect Idx Y",
        .reserved_ptr = 0,
        .opcode = 0x71,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_IDX_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x72] = {
        .name = "ADC",
        .description = "Add with Carry",
        .addr_mode_str = "Indirect ZP",
        .reserved_ptr = 0,
        .opcode = 0x72,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_ZP,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x73] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x73,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x74] = {
        .name = "STZ",
        .description = "Store Zero",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0x74,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x75] = {
        .name = "ADC",
        .description = "Add with Carry",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0x75,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x76] = {
        .name = "ROR",
        .description = "Rotate Right",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0x76,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x77] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x77,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x78] = {
        .name = "SEI",
        .description = "Set Interrupt Disable",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x78,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x79] = {
        .name = "ADC",
        .description = "Add with Carry",
        .addr_mode_str = "Absolute Y",
        .reserved_ptr = 0,
        .opcode = 0x79,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x7A] = {
        .name = "PLY",
        .description = "Pull Register Y",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x7A,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x7B] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x7B,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x7C] = {
        .name = "JMP",
        .description = "Jump",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0x7C,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x7D] = {
        .name = "ADC",
        .description = "Add with Carry",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0x7D,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x7E] = {
        .name = "ROR",
        .description = "Rotate Right",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0x7E,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x7F] = {
        .name = "BBR7",
        .description = "Branch on Bit Reset",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0x7F,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x80] = {
        .name = "BRA",
        .description = "Branch if Always",
        .addr_mode_str = "Relative",
        .reserved_ptr = 0,
        .opcode = 0x80,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x81] = {
        .name = "STA",
        .description = "Store Accumulator",
        .addr_mode_str = "Idx Indirect X",
        .reserved_ptr = 0,
        .opcode = 0x81,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IDX_INDIRECT_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x82] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x82,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x83] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x83,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x84] = {
        .name = "STY",
        .description = "Store Y Register",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0x84,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x85] = {
        .name = "STA",
        .description = "Store Accumulator",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0x85,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x86] = {
        .name = "STX",
        .description = "Store X Register",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0x86,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x87] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x87,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x88] = {
        .name = "DEY",
        .description = "Decrement Y Register",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x88,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x89] = {
        .name = "BIT",
        .description = "Bit Test",
        .addr_mode_str = "Immediate",
        .reserved_ptr = 0,
        .opcode = 0x89,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IMMEDIATE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x8A] = {
        .name = "TXA",
        .description = "Transfer X to Accumulator",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x8A,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x8B] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x8B,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x8C] = {
        .name = "STY",
        .description = "Store Y Register",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x8C,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x8D] = {
        .name = "STA",
        .description = "Store Accumulator",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x8D,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x8E] = {
        .name = "STX",
        .description = "Store X Register",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x8E,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x8F] = {
        .name = "BBS0",
        .description = "Branch on Bit Set",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0x8F,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x90] = {
        .name = "BCC",
        .description = "Branch if Carry Clear",
        .addr_mode_str = "Relative",
        .reserved_ptr = 0,
        .opcode = 0x90,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_RELATIVE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x91] = {
        .name = "STA",
        .description = "Store Accumulator",
        .addr_mode_str = "Indirect Idx Y",
        .reserved_ptr = 0,
        .opcode = 0x91,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_IDX_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x92] = {
        .name = "STA",
        .description = "Store Accumulator",
        .addr_mode_str = "Indirect ZP",
        .reserved_ptr = 0,
        .opcode = 0x92,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_ZP,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x93] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x93,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x94] = {
        .name = "STY",
        .description = "Store Y Register",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0x94,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x95] = {
        .name = "STA",
        .description = "Store Accumulator",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0x95,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x96] = {
        .name = "STX",
        .description = "Store X Register",
        .addr_mode_str = "ZP Y",
        .reserved_ptr = 0,
        .opcode = 0x96,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x97] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x97,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x98] = {
        .name = "TYA",
        .description = "Transfer Y to Accumulator",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x98,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x99] = {
        .name = "STA",
        .description = "Store Accumulator",
        .addr_mode_str = "Absolute Y",
        .reserved_ptr = 0,
        .opcode = 0x99,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x9A] = {
        .name = "TXS",
        .description = "Transfer X to Stack Pointer",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0x9A,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x9B] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0x9B,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x9C] = {
        .name = "STZ",
        .description = "Store Zero",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0x9C,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x9D] = {
        .name = "STA",
        .description = "Store Accumulator",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0x9D,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x9E] = {
        .name = "STZ",
        .description = "Store Zero",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0x9E,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0x9F] = {
        .name = "BBS1",
        .description = "Branch on Bit Set",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0x9F,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xA0] = {
        .name = "LDY",
        .description = "Load Y Register",
        .addr_mode_str = "Immediate",
        .reserved_ptr = 0,
        .opcode = 0xA0,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IMMEDIATE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xA1] = {
        .name = "LDA",
        .description = "Load Accumulator",
        .addr_mode_str = "Idx Indirect X",
        .reserved_ptr = 0,
        .opcode = 0xA1,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IDX_INDIRECT_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xA2] = {
        .name = "LDX",
        .description = "Load X Register",
        .addr_mode_str = "Immediate",
        .reserved_ptr = 0,
        .opcode = 0xA2,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IMMEDIATE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xA3] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xA3,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xA4] = {
        .name = "LDY",
        .description = "Load Y Register",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0xA4,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xA5] = {
        .name = "LDA",
        .description = "Load Accumulator",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0xA5,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xA6] = {
        .name = "LDX",
        .description = "Load X Register",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0xA6,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xA7] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xA7,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xA8] = {
        .name = "TAY",
        .description = "Transfer Accumulator to Y",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0xA8,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xA9] = {
        .name = "LDA",
        .description = "Load Accumulator",
        .addr_mode_str = "Immediate",
        .reserved_ptr = 0,
        .opcode = 0xA9,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IMMEDIATE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xAA] = {
        .name = "TAX",
        .description = "Transfer Accumulator to X",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0xAA,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xAB] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xAB,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xAC] = {
        .name = "LDY",
        .description = "Load Y Register",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0xAC,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xAD] = {
        .name = "LDA",
        .description = "Load Accumulator",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0xAD,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xAE] = {
        .name = "LDX",
        .description = "Load X Register",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0xAE,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xAF] = {
        .name = "BBS2",
        .description = "Branch on Bit Set",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0xAF,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xB0] = {
        .name = "BCS",
        .description = "Branch if Carry Set",
        .addr_mode_str = "Relative",
        .reserved_ptr = 0,
        .opcode = 0xB0,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_RELATIVE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xB1] = {
        .name = "LDA",
        .description = "Load Accumulator",
        .addr_mode_str = "Indirect Idx Y",
        .reserved_ptr = 0,
        .opcode = 0xB1,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_IDX_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xB2] = {
        .name = "LDA",
        .description = "Load Accumulator",
        .addr_mode_str = "Indirect ZP",
        .reserved_ptr = 0,
        .opcode = 0xB2,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_ZP,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xB3] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xB3,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xB4] = {
        .name = "LDY",
        .description = "Load Y Register",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0xB4,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xB5] = {
        .name = "LDA",
        .description = "Load Accumulator",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0xB5,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xB6] = {
        .name = "LDX",
        .description = "Load X Register",
        .addr_mode_str = "ZP Y",
        .reserved_ptr = 0,
        .opcode = 0xB6,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xB7] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xB7,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xB8] = {
        .name = "CLV",
        .description = "Clear Overflow Flag",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0xB8,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xB9] = {
        .name = "LDA",
        .description = "Load Accumulator",
        .addr_mode_str = "Absolute Y",
        .reserved_ptr = 0,
        .opcode = 0xB9,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xBA] = {
        .name = "TSX",
        .description = "Transfer Stack Pointer to X",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0xBA,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xBB] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xBB,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xBC] = {
        .name = "LDY",
        .description = "Load Y Register",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0xBC,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xBD] = {
        .name = "LDA",
        .description = "Load Accumulator",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0xBD,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xBE] = {
        .name = "LDX",
        .description = "Load X Register",
        .addr_mode_str = "Absolute Y",
        .reserved_ptr = 0,
        .opcode = 0xBE,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xBF] = {
        .name = "BBS3",
        .description = "Branch on Bit Set",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0xBF,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xC0] = {
        .name = "CPY",
        .description = "Compare Y Register",
        .addr_mode_str = "Immediate",
        .reserved_ptr = 0,
        .opcode = 0xC0,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IMMEDIATE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xC1] = {
        .name = "CMP",
        .description = "Compare",
        .addr_mode_str = "Idx Indirect X",
        .reserved_ptr = 0,
        .opcode = 0xC1,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IDX_INDIRECT_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xC2] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xC2,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xC3] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xC3,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xC4] = {
        .name = "CPY",
        .description = "Compare Y Register",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0xC4,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xC5] = {
        .name = "CMP",
        .description = "Compare",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0xC5,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xC6] = {
        .name = "DEC",
        .description = "Decrement Memory",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0xC6,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xC7] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xC7,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xC8] = {
        .name = "INY",
        .description = "Increment Y Register",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0xC8,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xC9] = {
        .name = "CMP",
        .description = "Compare",
        .addr_mode_str = "Immediate",
        .reserved_ptr = 0,
        .opcode = 0xC9,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IMMEDIATE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xCA] = {
        .name = "DEX",
        .description = "Decrement X Register",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0xCA,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xCB] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xCB,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xCC] = {
        .name = "CPY",
        .description = "Compare Y Register",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0xCC,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xCD] = {
        .name = "CMP",
        .description = "Compare",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0xCD,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xCE] = {
        .name = "DEC",
        .description = "Decrement Memory",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0xCE,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xCF] = {
        .name = "BBS0",
        .description = "Branch on Bit Set",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0xCF,
        .bytes = 4,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xD0] = {
        .name = "BNE",
        .description = "Branch if Not Equal",
        .addr_mode_str = "Relative",
        .reserved_ptr = 0,
        .opcode = 0xD0,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_RELATIVE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xD1] = {
        .name = "CMP",
        .description = "Compare",
        .addr_mode_str = "Indirect Idx Y",
        .reserved_ptr = 0,
        .opcode = 0xD1,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_IDX_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xD2] = {
        .name = "CMP",
        .description = "Compare",
        .addr_mode_str = "Indirect ZP",
        .reserved_ptr = 0,
        .opcode = 0xD2,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_ZP,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xD3] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xD3,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xD4] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xD4,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xD5] = {
        .name = "CMP",
        .description = "Compare",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0xD5,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xD6] = {
        .name = "DEC",
        .description = "Decrement Memory",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0xD6,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xD7] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xD7,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xD8] = {
        .name = "CLD",
        .description = "Clear Decimal Mode",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0xD8,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xD9] = {
        .name = "CMP",
        .description = "Compare",
        .addr_mode_str = "Absolute Y",
        .reserved_ptr = 0,
        .opcode = 0xD9,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xDA] = {
        .name = "PHX",
        .description = "Push Register X",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0xDA,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xDB] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xDB,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xDC] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xDC,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xDD] = {
        .name = "CMP",
        .description = "Compare",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0xDD,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xDE] = {
        .name = "DEC",
        .description = "Decrement Memory",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0xDE,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xDF] = {
        .name = "BBS5",
        .description = "Branch on Bit Set",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0xDF,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xE0] = {
        .name = "CPX",
        .description = "Compare X Register",
        .addr_mode_str = "Immediate",
        .reserved_ptr = 0,
        .opcode = 0xE0,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IMMEDIATE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xE1] = {
        .name = "SBC",
        .description = "Subtract with Carry",
        .addr_mode_str = "Idx Indirect X",
        .reserved_ptr = 0,
        .opcode = 0xE1,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IDX_INDIRECT_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xE2] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xE2,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xE3] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xE3,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xE4] = {
        .name = "CPX",
        .description = "Compare X Register",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0xE4,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xE5] = {
        .name = "SBC",
        .description = "Subtract with Carry",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0xE5,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xE6] = {
        .name = "INC",
        .description = "Increment Memory",
        .addr_mode_str = "ZP",
        .reserved_ptr = 0,
        .opcode = 0xE6,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xE7] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xE7,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xE8] = {
        .name = "INX",
        .description = "Increment X Register",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0xE8,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xE9] = {
        .name = "SBC",
        .description = "Subtract with Carry",
        .addr_mode_str = "Immediate",
        .reserved_ptr = 0,
        .opcode = 0xE9,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_IMMEDIATE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xEA] = {
        .name = "NOP",
        .description = "No Operation",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0xEA,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xEB] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xEB,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xEC] = {
        .name = "CPX",
        .description = "Compare X Register",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0xEC,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xED] = {
        .name = "SBC",
        .description = "Subtract with Carry",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0xED,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xEE] = {
        .name = "INC",
        .description = "Increment Memory",
        .addr_mode_str = "Absolute",
        .reserved_ptr = 0,
        .opcode = 0xEE,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xEF] = {
        .name = "BBS6",
        .description = "Branch on Bit Set",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0xEF,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xF0] = {
        .name = "BEQ",
        .description = "Branch if Equal",
        .addr_mode_str = "Relative",
        .reserved_ptr = 0,
        .opcode = 0xF0,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_RELATIVE,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xF1] = {
        .name = "SBC",
        .description = "Subtract with Carry",
        .addr_mode_str = "Indirect Idx Y",
        .reserved_ptr = 0,
        .opcode = 0xF1,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_IDX_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xF2] = {
        .name = "SBC",
        .description = "Subtract with Carry",
        .addr_mode_str = "Indirect ZP",
        .reserved_ptr = 0,
        .opcode = 0xF2,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_INDIRECT_ZP,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xF3] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xF3,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xF4] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xF4,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xF5] = {
        .name = "SBC",
        .description = "Subtract with Carry",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0xF5,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xF6] = {
        .name = "INC",
        .description = "Increment Memory",
        .addr_mode_str = "ZP X",
        .reserved_ptr = 0,
        .opcode = 0xF6,
        .bytes = 2,
        .addr_mode = QE6502_ADDR_MODE_ZP_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xF7] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xF7,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xF8] = {
        .name = "SED",
        .description = "Set Decimal Flag",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0xF8,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xF9] = {
        .name = "SBC",
        .description = "Subtract with Carry",
        .addr_mode_str = "Absolute Y",
        .reserved_ptr = 0,
        .opcode = 0xF9,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_Y,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xFA] = {
        .name = "PLX",
        .description = "Pull Register X",
        .addr_mode_str = "Implied",
        .reserved_ptr = 0,
        .opcode = 0xFA,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_IMPLIED,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xFB] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xFB,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xFC] = {
        .name = "ILL",
        .description = "Illegal opcode",
        .addr_mode_str = "Illegal",
        .reserved_ptr = 0,
        .opcode = 0xFC,
        .bytes = 1,
        .addr_mode = QE6502_ADDR_MODE_ILLEGAL,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xFD] = {
        .name = "SBC",
        .description = "Subtract with Carry",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0xFD,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xFE] = {
        .name = "INC",
        .description = "Increment Memory",
        .addr_mode_str = "Absolute X",
        .reserved_ptr = 0,
        .opcode = 0xFE,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ABSOLUTE_X,
        .is_cmos_extension = 0,
        .reserved_data = {0, 0, 0, 0}
    },
    [0xFF] = {
        .name = "BBS7",
        .description = "Branch on Bit Set",
        .addr_mode_str = "ZP Relative",
        .reserved_ptr = 0,
        .opcode = 0xFF,
        .bytes = 3,
        .addr_mode = QE6502_ADDR_MODE_ZP_RELATIVE,
        .is_cmos_extension = 1,
        .reserved_data = {0, 0, 0, 0}
    },
};

#endif /* QE6502_OPCODES_H */
